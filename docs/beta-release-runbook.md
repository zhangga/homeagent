...
Codex 双执行模式已具备底层策略、证据、本机确认、绑定 epoch/范围、计划 v2、逐进程门禁、页面确认/撤销和逐 Agent 准备检测；完整验收未完成，不能据此发布“Windows 完全访问可用”。
2026-09-14 的 [最小真实验收](codex-dual-execution-mode.md#133a-不接飞书的最小真实验收入口) 已验证 Windows / Codex 0.154.0 的认证复用、完全访问 start/fork、文件编辑和撤销阻断。它直接使用真实 Core/LLM、空测试 SkillCatalog 和本地绑定元数据，不是飞书送达/话题持久化、完整 Skill、管理页面真实 Provider 或重启衔接验收。
发布含管理页入口的版本前，必须通过 [双模式实施文档](codex-dual-execution-mode.md#13-验证矩阵与执行方式) 的
本机确认原子发布/撤销、scope/epoch、旧 full 迁移、归档脱敏与每次模型启动门禁；两种模式独立验收。
实际 Windows 身份复用、start/fork、工具执行和重启衔接需要单独授权的 live 证据，假 CLI 通过不能替代。
离线浏览器门禁可使用 [双模式浏览器验收入口](codex-dual-execution-mode.md#132a-离线浏览器验收)：真实管理页配临时数据和假 Provider，检查桌面/窄屏的确认、检测、撤销和模式切回。必须验证浏览器确认表单发送同站 Origin；不可通过放行 `Origin: null` 消除 403。
2026-09-14 Windows Edge 离线流程已通过，但不替代 macOS/Linux、真实移动设备或真实 Provider。固定质量评测使用空 SkillCatalog，避免读取操作者本机工具影响门禁耗时；生产 Skill 校验保持不变。
页面门禁另需覆盖：草稿不授权、独立风险确认、Task 默认关闭、真实完整范围、跨站/伪造转发头/重复字段/超长表单拒绝、陈旧版本/范围/实例令牌拒绝、确认来源由后台身份决定，以及原子发布/撤销写失败。
默认启动/页面检测不得执行隔离探针；完全访问恢复与旧沙箱轮询不得调用 sandbox setup，隔离诊断须显式请求。机器诊断不能显示“完全可用”，也不能替代按 Agent 配置的准备状态和真实模型证据。
逐 Agent 检测门禁另覆盖：草稿不替换发布配置、模式/身份/Workdir/授权/范围/Skill 变化失效、异步晚结果不覆盖、退出取消、两路并发/同 Agent 合并、60 秒过期/时钟回拨和 256 项淘汰。
验证两个 Agent 的相同 Skill 刷新不会延长或错误废弃旧有效结果；Skill 资源在检查中修改必须失败。GET/健康读取不能启动 Provider；公开汇总仅有数量、不含 Agent/群名、路径、确认 ID 或诊断原文。
`agentExecution` 汇总只覆盖实际启用的对应工作流：已知配置失败阻止 `/readyz`，未知/过期为 degraded，不单独阻断；断开的群和未选择的模式不加入所需能力，启用的完全访问 Task 必须有 Task 本机确认。不得用前台准备通过掩盖后台或 Provider 运行失败。
新增回归门禁包括：撤销先提交不启动、进程先注册则取消、撤销落盘失败不误报成功、routing 后发布保留旧计划/routing 后撤销停止、
旧 full 队列/待审批恢复停止、手动 Task/WorkAction 重试冻结计划及时限、高权限重试重新审批，以及 Task 后 Dream 不继承执行权限。
Provider 启动许可为同步 Interface；未来若 Store 改为异步提交或跨 Worker 调用，须重新建立并验证启动/撤销串行契约，不能直接给许可加 `await`。
确认保留门禁需覆盖每 Agent 100 条、集合 10,000 条与紧凑 UTF-8 JSON 16 MiB：新发布只淘汰无引用旧记录，保留当前发布、所有非终态 Run 和在途临时调用。
验证原生预检/普通问答没有持久化 Run 时仍受保护、并发引用独立释放、错误退出释放引用、未知引用/关闭后拒绝清理，及清理与发布/回滚写失败不产生部分生效。
清理后的终态 Run 和 Agent revision 重启仍可读，旧引用无法恢复执行权限；支持平台 crash-recovery 和真实恢复验收不能以 Windows 假 Provider 测试替代。
本阶段另需验证 Chat 文件 v6/v7→v8 后旧 v1 计划的已送达话题能继续 fork，计划 v2 使用独立兼容摘要；旧证据保持未知、新证据重启不丢失，
以及未来格式拒绝读取。Task v12→v13 不改写旧审批或计划；损坏/未来运行文件不能按空历史覆盖。
Registry 无版本→v1、飞书绑定 v1/v2→v3 必须先原子保存 epoch；策略 A→B→A、解绑重连、导入均不能复活旧确认，写失败不改变内存。
还需验证 Agents v4→v5 保留旧快照且不补造确认、正常同机重启保留确认、
旧备份恢复先撤销确认且写失败不能启动 Store。Space 归档 v20 的专用历史计划 DTO 不能导出本机确认、scope、epoch 或执行证据；恢复到新目录必须重新确认，不能直接执行/重试历史 DTO。

## 1. 候选代码本地预检

聊天原文自动入库的发布验收还需覆盖：来源群内显式请求、冻结可写权限/完全访问确认、真实 `lark-im` JSON 转换、
以及从测试群明确指定另一个来源群：先搜索真实目标再取数，Raw 只写发起群 Space、保留真实来源群与名称，目标 Space 不被创建或修改。
验证同名歧义提示、错误当前群替换被拒绝、未声明外部来源时拒绝跨群交接、不同来源同消息 ID 不误去重、各目标进度隔离、
同名目标 ID 变化不复用基线，以及归档仍保留 Raw 来源但排除本机进度。查询和入库范围不同不能阻断已明确授权的查询。
同话题续作需覆盖：首轮要求保存原文，重启后仅追问“再提炼最近两周”，仍向发起群 data 目录入库并返回实际回执；每轮交接路径不同。
首轮需同时覆盖实际表达“提炼下最近两周的主要内容，记录相关信息”，以及“保存相关数据”“保存下来”；不能要求额外添加“原始／原文”关键词。普通查询与只保存总结不自动收录群原文。
Raw 检索验收需覆盖：新话题只查询当前群已入库 Raw，提供原文摘录、来源群、消息时间和真实 Raw ID；不调用飞书或读取 Workdir，不附加新的入库回执。无知识页时仍可查；旧 SQLite 回填、索引删除重建、SQL 写失败后重开、撤回及 held/excluded 排除、其他 Space 不混入、候选/正文边界和无匹配结果均需验证。
同时验证改群、取消保存、无关回复、新话题及当前 Run 之后的请求不会错误改变本轮入库要求；未完成覆盖不能用本地 baseline 文件推进增量。
正文/作者/时间/线程追溯、同消息重复请求、正文解析失败与旧主题覆盖提示。使用超大附件引用确认不会下载二进制，
核对回执计数与 Raw journal；覆盖无附件消息省略 `attachments`、正常正文与已撤回缺口混合时的入库及重试去重。
确认本地周报和交接文件显示为本机路径，真实消息证据仍可点击；另验证撤回、取消、授权撤销、Workdir 替换、8 MiB/1,000 条边界、部分写入失败及重启后去重。
补漏验收需包含：批量补读失败后的单条补读、`Unknown method` 命令核对、旧根消息在窗口内新增回复、搜索分页封顶、
线程时间边界和搜索索引延迟；不能把搜索命令成功视为全部正文完整。验证已编辑消息另存版本并保留旧引用，Dream 不将快照赋予人工纠错优先级。
查询进度验收需覆盖：空窗口完成、旧缺口与新窗口交错、Raw 写失败、最后进度提交失败、重启后补查、状态损坏/未来版本/链接拒绝，
以及 1,000 个失败 ID / 512 KiB 边界、绑定变更不复用、Space 删除清理与归档不携带本机进度。明确周报仍查询完整用户窗口，增量回看边界且不跳过缺口。
在 Ubuntu/macOS 运行恢复门禁；Windows 假 Provider 测试不能替代这些真实飞书与持久化恢复验收。
...
6. 导出 `homeagent.space v20` 并恢复到全新目录，确认 open/resolved 状态、revision、处置说明与幂等键保持；
   v1–v17 归档应归一化为空反馈集合，跨 Space、伪造处置关系或超过 5000 条反馈的 v18–v20 归档必须 fail-closed。
...
3. 导出 `homeagent.space v20`，确认原文件载荷、摘要、字节数和 Raw attachment 双向一致；篡改 base64、摘要、
...
9. 导出并恢复该空间，确认格式为 `homeagent.space v20`，Agent 发布历史、运行所引用的 revision、完整 Skill 证据、跳过原因、工作上下文、WorkAction/checkpoint/验收审计、自动续作策略、Raw 准入状态、分层知识地图、本地 Agent 知识反馈、完整原文件及 Wiki Maintenance 最近完成时间/有界摘要保持不变；待验收动作必须阻止导出与删除。Provider 原生会话 ID/路由不得出现在归档，恢复后同一飞书话题必须新建 Provider 会话。验收前 Raw 必须为 `held` 且不能被 Dream、强制重跑、隔离重试或人工重新提炼读取；接受后才变为 `ready`，驳回、取消或失败后必须为 `excluded`。
...
### 4.2 v10–v19 真实归档迁移验收

从对应历史版本各准备一份脱敏的真实 `homeagent.space v10`、v11、v12、v13、v14、v15、v16、v17、v18、v19 归档；不得只修改 JSON 的 `version` 字段伪造。
...
4. 将迁移后的空间重新导出，确认版本为 `homeagent.space v20`；重启 HomeAgent 后再次导出，比对知识及分层地图、Agent revision、Task/Chat Run、工作上下文、WorkAction/checkpoint、Raw 准入状态、自动续作策略、
   审批/通知/重试/用量审计、提醒和学习数据的数量与关键 ID。
5. 把该 v20 归档导入第二个全新数据目录，确认没有重复通知、自动续跑、重复动作 checkpoint、重复原始材料、悬空工作关联、未验收 Raw 污染 Wiki、丢失原文件或悬空质量 trace。把原归档摘要、两次 v20
   导出摘要及逐项结果附到发布记录。
...
- 自动与真实崩溃恢复均通过；
- v10、v11、v12、v13、v14、v15、v16、v17、v18、v19 十份真实归档均完成独立迁移、v20 再导出、重启和二次恢复，比对记录已归档；
