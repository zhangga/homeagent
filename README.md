...
管理页上传 ZIP 时会自动解压包内各级目录的 UTF-8 文本（TXT、Markdown、CSV、JSON、JSONL/NDJSON、LOG），
分别登记到所选 Space 的 Raw，保留压缩包名称与内部路径；超过 200,000 字符的包内正文会分段，避免只提炼文件开头。
原 ZIP 与成功读取的文本原文件完整保存、可下载，未识别、损坏、加密或超限的成员会在 ZIP 的 Raw 中说明。
包内单文件沿用 20 MiB 原文件限制，一次解压沿用上传总预算 400 MiB；嵌套 ZIP 不递归展开。
勾选「导入后立即提炼」时按每批最多 40 条 Raw 处理本次资料；否则进入现有自动提炼队列。
耗时较长时，可另开「运行状态 → 提炼任务」查看排队、资料分析、知识页生成、保存与索引阶段，
以及当前知识页、来源 Raw、分段与 ZIP 批次进度、耗时和最近进展。运行中每 3 秒自动更新，空闲时每 10 秒检查。
模型调用期间会明确显示等待结果，不把经过的时间当作内容进度；失败时可跳转到该空间的提炼失败记录。
详情保留本次服务启动后的当前任务与最近 50 条结束记录，重启后清空；正式 Raw、知识页和失败恢复记录不受影响。
...
开发中的 Codex 双模式已接通管理页选择、确认发布、撤销及逐 Agent 准备检查（P4），并通过 Windows Edge 的离线桌面/手机宽度交互验收。
2026-09-14 已在 Windows / Codex CLI 0.154.0 的一次性实例验证登录复用、完全访问回答、原生 start/fork、文件编辑和撤销阻断；完整 Skill、真实飞书与重启衔接仍未验证，不代表既有生产服务已恢复。
需经授权重跑时，使用 [最小真实验收入口](docs/codex-dual-execution-mode.md#133a-不接飞书的最小真实验收入口)；它不接飞书、不修改现有 Agent，结束后清理测试数据和认证副本。
在 Agents 的执行配置中选择“本机完全访问（无沙箱）”，明确使用 `full` 和有效 Workdir，再点击“继续：确认发布”。
独立确认页会列出来源版本、模型、Instruction、完整 Chat 绑定范围和无沙箱风险；须手动勾选后提交才产生新发布版本及本机确认。
切换模式时发布按钮会立即更新；手机详情抽屉可独立滚动到检测与撤销操作。确认页保留同站表单 Origin，仍拒绝跨站或 `Origin: null` 请求。
Task 开关默认不勾选，即使勾选，每次高权限 Task 仍须单独审批。保存草稿、新建配置、浏览确认页都不会授权；回滚完全访问也必须重新确认。
右侧显示已确认/待确认范围并提供“撤销完全访问”。切回隔离时须明确选 `read-only` 或 `write` 后发布，不自动修改为受限权限。
发布后点击“检测当前发布配置”：检查实际 Workdir、完整 Skill、CLI 原生命令/MCP/身份及该模式的准备条件；不调用模型，也不创建授权或发起 Windows 沙箱设置。
页面区分待检测、检测中、准备通过和未通过，始终另列“实际模型调用未验证”。结果仅保留最多 60 秒；发布、授权/范围变化、Workdir 替换、CLI/凭据文件变化或 Skill 刷新会使旧结果失效。
此诊断缓存最多 256 个 Agent、同时检测两个；重复检测会合并。文件元数据无法即时观察 OS keyring/MDM 变化，实际执行仍重新检查，不能把缓存当作启动许可。
健康汇总只读取所需工作流的状态，不启动 Agent 探针：已知不可用会阻止 `/readyz`，待检测/过期只标记 degraded，不单独阻止就绪；后台学习、Dream 和 Provider 最近运行故障仍独立判断。
Agents 文件新写入 v5：完整发布与确认同文件原子提交，撤销和切回隔离会使旧确认失效；从 `.bak` 恢复时先持久化撤销全部有效确认，正常同机重启则保留。
发布达到确认数量或容量上限时，Core 只清理不再使用的旧确认，并与新版本一起原子保存；当前发布、排队/待审批/运行中的 Run，以及预检和普通问答的在途引用均保留。
没有可安全清理的记录或无法确认引用状态时拒绝发布。被清理确认对应的终态历史仍可查看，但旧引用不能恢复执行授权；清理不会删除 Agent 版本、Raw 或知识页。
旧 Codex `full` 只保留历史意图，不会自动授权；新配置不能省略执行模式而选择 `full`。旧排队/待审批项及确认失效的运行会明确停止；
撤销、解绑或响应范围变化会取消在途完全访问调用，不能撤回已经发生的外部效果。手动重试保留原计划、Skill 和执行时限，高权限仍须再次人工审批；要采用新配置应新建运行。
Task 后的提炼使用独立无工具调用，不继承完全访问。完全访问历史暂不支持质量重评；运行详情另显示冻结确认当前是否有效，发布历史显示各版本的模式意图，
进度见 [双模式实施文档](docs/codex-dual-execution-mode.md#12-分阶段实施与完成条件)。不要手改数据文件开启此能力。
旧 Codex `full` 不被视为新模式授权，实际 Provider 入口会拒绝没有明确模式及内部启动许可的完全访问调用；
不能只改 JSON、权限下拉或绕过探针来启用。原生话题按已发布且已确认的执行契约运行；隔离模式下 Windows 的 `75` 仍表示隔离未通过。
新的执行证据区分“进程已启动”与“模型调用已验证”；沙箱不适用不等于沙箱检查通过。
绑定或回复策略改变后，即使再改回原值，也需要重新确认；名称和健康检测变化不影响范围。绑定先保存，确认发布失败时仍为未授权，不能启动完全访问。
新 Run 使用计划 v2，分别冻结 Chat 范围或 Task 确认引用；Task 仍需逐次人工批准。Space 归档只保留不可执行的历史计划，不转移本机确认、范围摘要或绑定版本号。
当前写入格式为 Registry v1、飞书绑定 v3、Chat v8、Task v13；保留对应旧格式读取及原 v1 计划，不用当前配置改写历史。
损坏或未来版本的运行文件拒绝启动，不能作为空历史覆盖；升级前保留备份，升级后不要直接用旧版本打开已更新的数据目录。
...
- **空间 / 知识**：空间列表、分层知识地图、知识页、原始条目、问答测试、手动触发提炼，以及提炼失败记录的单条/批量恢复。
  ...
  - 空间详情页可一次导入最多 20 份任意类型的本地文件，每份不超过 20 MiB。原文件按字节完整写入当前 Space 的 `raw/sources/`，不受 200,000 字符提取上限影响；文件名和原始内容 SHA-256 进入可追溯的 `source=manual` Raw。受支持的 UTF-8 文本最多提取 200,000 字符用于提炼；其他格式、无效编码或空文件仍完整保存，只是不生成替代正文。可只登记后等待夜间提炼，也可立即且仅提炼本批资料，并可在 Raw 详情页直接下载原文件。
...
- **Agents**（三栏工作台）：左侧选择 Agent，中间编辑配置，右侧查看真实的 CLI 状态、当前空间/飞书群绑定和该 Agent 最近处理的 Chat / 研究任务运行。Chat 记录会固定归属到实际处理它的 Agent，并可从 Recent runs 进入对应的原始消息详情。支持新建 / 删除，以及配置 **名称、Provider、Instruction（人格，会注入到回答）、Model、推理强度、Visibility、Permission、Workdir**；旧版 Pinned Skill 绑定会保留兼容，但不再限制运行时能力。编辑先保存为草稿，显式发布后才影响未来运行，发布历史不可变并支持通过新版本回滚，避免在途运行被原地改写。
  ...
  - **不可用时可就地恢复**：Agent 详情右侧会显示恢复卡片。Codex 账号连接异常时，“恢复 Codex”会先尝试复用本机控制台登录，仍不可用则进入官方授权。默认检测只检查命令、身份和原生命令支持，不自动执行沙箱探针。隔离模式可显式点击“检测隔离能力”；仅明确报告 elevated 后端未就绪时提供 Windows 系统授权入口，通过后显示“机器隔离探测已通过”，不等于实际 Agent/模型调用已验证。完全访问页只重新检测 CLI 连接，不发起沙箱设置或受限探针。CLI 缺失时保留同机检查命令和设置入口。
  - **Skills 自动加载**：飞书群绑定团队空间，空间再绑定 Team Agent；个人空间绑定 Personal Agent。每次普通聊天或任务启动时，HomeAgent 都会扫描当前 Provider 可用的本机 Skill 目录，把完整兼容目录交给 Agent，再由 Agent 按当前请求加载相关 Skill，无需逐个固定。旧 Agent 的 Pinned Skill 仅作为兼容配置保留，不再形成能力白名单。
    普通聊天要求查询或整理外部来源时，回答阶段会指示 Agent 先使用相关 Skill 获取证据，即使命中了只记录请求或缺口的知识页，也不能把它当作已经完成查询。没有知识页支撑的工具答案直接返回，不额外启动一次通用回答。该行为仍取决于本轮冻结 Skill、实际工具权限与来源账号授权；本机完全访问不代表已获跨 Space 查询授权，工具不可用时须说明实际阻碍。明确要求仅依据知识库的查询不启用这项补查指引。
    **聊天原文自动入库**：在已启用的群内请求提炼、提取、整理、总结或汇总群聊历史时，默认同时保存本轮取得的来源原文；显式要求“不要保存／不要入库／只保存总结”时退出自动收录。具有有效 `write` 或已确认 `local-full-access` 权限及 Workdir 的 Chat Run 会得到本轮专用 JSON 交接路径。Agent 按实际 Skill 查询结果转换消息原文、发送者、来源时间、消息 ID／链接、线程关系与附件引用；HomeAgent 在回答生成后校验整份文件，再写入发起请求群的 Raw journal，并在回复末尾追加实际入库回执。用户明确指定其他来源群时，也归入发起请求群的 Team Space，其成员可检索导入的原文；不创建或修改来源群的 HomeAgent Space。私聊自动入库不在此流程内。
    查询来源、回答所在群和 Raw 所属 Space 分别处理。例如在“测试 AI”群请求 `飞书群：PST线上应急处理小组 提炼最近一周，并记录原始数据`，先按名称搜索 PST 群，唯一匹配后使用其真实群 ID 取数，Raw 归测试群，来源 metadata 保留 PST 的名称和 ID。后台从当前及同话题已保存的用户请求解析单一群名／ID 范围；群名请求的交接必须附实际返回的 `chatName` 并匹配（忽略空白及全半角差异），ID 请求必须匹配该 ID。未声明外部来源时只接受当前群；不能用交接内容自行扩大范围、替换来源 ID 或读取其他 Space 的既有知识库。群名解析与内容仍是 Agent 提交的来源快照，不是服务器独立验证。多群或无法识别的表达式不自动扩大入库范围；查询及本地保存可继续，入库单独报告原因，不要求用户重复确认已唯一解析的群 ID。
    **同话题续作**：群消息的“提炼／提取／整理／总结／汇总”默认触发原文入库；“记录相关信息”“保存相关数据”“保存下来”同样触发，不要求用户使用“原始／原文／Raw”等术语。单纯“查询聊天记录”仍不自动收录，明确不保存或只保存总结时退出自动收录，并在同一话题后续保持该选择，直至用户再次明确要求保存。首轮默认或明确要求保存后，在同一 Codex 飞书话题追问“再提炼最近两周”或“继续补查”，沿用此前来源群与保存要求；本轮指定新来源或取消保存时以本轮为准。后台依据保留的 Chat Run 用户输入及话题归属恢复该要求，正常重启后仍有效，不把历史聊天全文拼进 Provider prompt，也不跨话题继承。每轮生成独立交接文件；正式 Raw 由 HomeAgent 写入实际 data 目录下发起群的 `raw/records/`，不能以 Workdir 内的周报、消息 JSON 或 `baseline.json` 代替入库和基线提交。
    **查询已入库原文**：明确要求查询知识库中的 Raw、核验原文或给出 Raw ID 时，后台直接检索可读 Space 的 Raw 全文索引，不依赖知识页是否已生成，也无需 Agent 调用飞书 Skill 或读取 Workdir 导出文件。先用 FTS 选取最多 20 条候选，在总计 120,000 字符、单条 12,000 字符的原文范围内核验，最多返回 5 条原文摘录；摘录必须与存储正文逐字一致，来源群、消息时间和 Raw ID 由后台记录生成。未命中会明确说明本次未核验到，不宣称全库不存在。查询“已入库”不会触发新的入库交接，也不会覆盖同话题既有的外部来源保存要求。
    每次交接最多 8 MiB、1,000 条消息及失败 ID，每条正文最多 100,000 字符、32 个附件引用。无附件消息的 `attachments` 可省略或写为 `[]`；已声明的缺失正文不会阻止有效消息入库。只保存附件引用、名称、类型和已知大小，不自动下载图片、视频、大日志等附件，也不把引用说成文件已备份。原文以 `source=manual` 标记为 Agent 提交的来源快照，区别于 Connector 直接接收的消息；知识页在后续 Dream cycle 中生成，原始记录入库不代表已提炼。交接 JSON 留在 Workdir 供核验，不进入 Provider 原生会话映射。
    飞书回复中的本地交接文件、周报等显示为可复制的本机路径，不作为网页下载链接；需在运行 HomeAgent 的电脑上访问。真实网页及消息证据链接保持可点击。
    按 Space、来源群和消息 ID 去重；已直接收录的未编辑消息及已撤回消息会跳过。来源返回有效 `update_time` 时，同 ID 的编辑会保存为独立 Raw 版本，重复版本跳过，旧原文与引用保留；缺少编辑时间时不猜测版本顺序。Dream 按同一消息的来源编辑时间区分新旧，不将多个版本算作多起事件，也不将群聊快照视为管理员纠错。无效文件在写入前整批拒绝；持久化中途失败会明确报告未完成，已写入的 Raw 保留，重新请求时去重。取消、撤销授权、改变群绑定或 Workdir 后拒绝交接。
    取数指令要求按冻结 Skill 和本机 CLI 帮助核对命令：指定群名先用 `+chat-search` 解析目标；正文失败先用 `+messages-mget` 分批补读，再单条补读一次；旧主题通过限定目标群、同一时间窗口的无关键词 `+messages-search` 补查，并展开发现的线程。线程回复按自身时间过滤，所有分页必须检查实际结束标志；达到 `--page-all` 上限不能视为完整。搜索索引可能延迟，这套覆盖声明仍来自 Agent 的查询结果，不是 HomeAgent 对飞书历史的独立核验。
    本机 Space 下的 `.chat-source-progress.json` 保存待补查窗口、失败消息 ID 和已完成查询截止时间，采用原子写入；最多保存 1,000 个失败 ID，文件上限 512 KiB。先登记待处理窗口，再写 Raw，全部落盘且主消息、线程、旧主题检查均完成、无失败消息时才推进增量起点。较新的局部查询不能消除旧缺口，重启后继续使用该状态；文件损坏时拒绝自动交接，不按空状态覆盖。继续/增量请求需重叠回查时间边界；明确要求“最近一周”仍查询完整一周。超出本次指定窗口的历史缺口会提示，不自动扩大正文查询范围。历史编辑仅在消息被再次查询且返回编辑时间时发现，不保证扫描所有旧消息的编辑。
    明确来源目标的进度按声明的群名或 ID 存在 `.chat-source-progress/<摘要>.json`；与当前群默认进度分开，并记录实际 `sourceChatId`。同名目标解析到不同 ID 时重新建立基线，不沿用旧群的进度；通过其他名称或 ID 重新指定同一群时可能重新查询，Raw 仍按真实来源 ID 去重。失败消息不会作为正常正文入库，附件不自动下载。查询进度是本机工作流状态，不进入 Space 归档；群绑定或 Agent 绑定变更后旧进度不复用，删除 Space 时一并移除。没有实际入库回执，Agent 不应声称已入库或手工写运行中的数据目录。补查在下一次用户请求内执行，不创建后台定时任务。
    Codex 单个 Skill bundle 最多 16 MiB / 50,000 个条目；完整目录的临时副本总预算为 128 MiB / 200,000 个条目，逐个捕获、散列核验后复制。总预算超限会停止整次调用并清理副本，不会删除尾部 Skill 后继续运行。隔离模式的原生调用只为本轮独占、已验证且仅含这些副本的临时根目录授予读取权限，避免逐个路径重复出现在 Windows 命令行中；不会授予本机 Skill 源目录或整个临时目录的访问权。完全访问仍使用冻结副本，但不提供文件访问隔离。
  - **仍隔离环境规则**：运行不会继承用户级规则、Hooks、Plugins、Codex 自动 Skill 发现或无关 Provider 会话历史；唯一例外是显式匹配到同一飞书话题、且冻结执行配置兼容的 Codex 原生父会话。HomeAgent 会在每次调用前按冻结摘要捕获 Skill bundle，复制到本轮私有临时目录并对副本复验，只把这份副本的唯一 `name → SKILL.md` 映射交给 Codex；live 目录变化会 fail-closed，项目 `.agents/.codex`、ambient 用户目录和管理层 Skill 不会被自动注入。两种模式的路由/分类和最终生成都使用同一冻结契约，只有最终生成携带原生会话；fork prompt 会声明忽略历史 turn 中已清理的旧临时 Skill 路径。每个原生 turn 都会无模型验证 MCP 严格为空；隔离模式另外以 exact filesystem profile 验证允许目录可读、隔离 `CODEX_HOME` 与仅受根拒绝规则保护的数据目录 sentinel 均不可读。完全访问不运行这些文件隔离探针，但仍检查身份、版本、MCP、Workdir、Skill 和本机授权。门禁不成立时实际调用 fail-closed，不自动降级；机器级诊断通过不能代替本次 Agent 检查。任务和 Chat Run 会记录冻结的 Skill 列表，方便审计。
  ...
  - **群话题不自动降级**：Codex 原生会话预检失败后不会再次发起无话题记忆的普通调用，也不会自动把 `full` 改成 `write`。旧或未确认 `full` 的配置/授权问题会明确提示；只读需求选 `read-only`，需要写工作目录才选 `write`，保存并发布后仅影响新运行。两种受限权限仍须通过平台隔离检查；显式完全访问则须另行确认无沙箱风险，不能称作修好了 Windows 沙箱。重试继续使用原冻结计划；需要采用新权限或全量 Skill 目录时应新建一次请求。
    管理页与回复会区分具体预检原因：退出码 73 表示 Codex 状态目录仍可读，74 表示授权测试目录不可读，75 表示受保护数据目录仍可读。75 不是未登录或缺少 Skill，也不等于需要重新完成 UAC 授权；这台机器未证明根目录拒绝规则生效，必须保持隔离模式的原生话题不可用。仅当明确报告 elevated 后端未就绪时提供对应设置操作；重新检测不得把“CLI 可连接”显示为“原生话题已恢复”。
  - **Chat 执行证据**：从 Agent 的 Recent runs 打开 Chat Run 详情，可查看冻结权限、Skill 列表以及 Codex 报告的已完成工具事件。每次运行最多保存 16 次 Provider 输出，每次最多 32 个事件；超限明确标记截断。简单、可明确识别的 `lark-cli` 只读命令额外记录操作类别、显式 user/bot 身份、CLI 报告身份、成功标志、返回条数与分页状态；复杂脚本、编码命令和缺失字段保持未知。命令退出成功不等于业务完成，搜索到群也不证明消息读取成功。证据不保存完整命令、输出正文、群名、路径或凭据，仅留在本机管理运行记录，不进入 Space 导出、健康输出或模型提示词。旧运行不补造证据；中断或预检失败时可能没有可采集事件。
    新运行的已识别准备失败另保留固定的阶段/原因码及退出码，或 Skill 超限时的请求数/已准备数；即使没有 Provider 工具输出，也能说明这次停在哪项检查。它们不是原始日志，不会把未识别的 Provider 文本直接展示给用户。
  ...
  - **Model 随 Provider 变化**：切 Provider 时 Model 下拉自动换成该 provider 的维护清单（CLI 无“列模型”接口）；Codex 当前提供 `gpt-6-astra / gpt-5.6-sol / gpt-5.6-terra / gpt-5.6-luna / gpt-5.5 / gpt-5.4 / gpt-5.4-mini / gpt-5.3-codex-spark`。其中 `gpt-5.6-sol` 是 GPT-5.6 Sol 的完整模型 ID；HomeAgent 日常问答优先选择较快、成本更低的 `gpt-5.6-luna`，复杂研究可选择 `gpt-5.6-terra` 或 `gpt-5.6-sol`。
  - **推理强度按 Agent 配置**：Codex Agent 可选择继承默认值，或从当前模型支持的档位中选择；GPT-6 Astra（`gpt-6-astra`）支持 `low / medium / high / xhigh / max / ultra`（按 Codex 模型目录），不支持 `none`；GPT-5.6 系列支持 `none / low / medium / high / xhigh / max`，旧模型不会显示不支持的档位。此配置用于普通 Chat 和显式 Codex 任务；其他 Provider 暂不传递。
  ...
  - **Chat / 任务权限会真实映射到 CLI 沙箱**：`read-only` 开启只读工具并禁止写入，`write` 以 Workdir 为工作根目录并启用 Provider 的工作区写入模式，`full` 会绕过 Provider 沙箱。`write/full` 必须配置存在的 Workdir；高权限任务仍需持久化人工审批。Codex 的旧或未确认 `full` 不可执行；开发中的显式本机完全访问另需发布确认及逐进程授权，不能被描述为已隔离。未配置 Workdir 的只读话题使用本轮专用空目录，不会回退到 HomeAgent 当前工作目录。话题 Workdir 与 HomeAgent 数据目录任一方向重叠仍拒绝；在完全访问模式下，这项检查只是防误操作，不能限制 Provider 读取其他目录。
...
- **数据治理**：按空间导出 `homeagent.space v20` JSON 完整备份（知识页及系统生成的分层地图、原始记录及其动作验收准入状态、按 SHA-256 校验的完整原文件、工作上下文、续作动作/checkpoint/验收审计/策略及其证据关联、人工治理审计、本地 Agent 知识消费反馈及处置记录、撤回标记、任务及 Chat 运行历史、冻结执行计划、Agent 发布历史、审批/重试/通知/用量审计、Skill 证据、相关质量 trace 与封闭的重评记录、提醒、学习计划、主题路线、多来源材料及课程历史、Wiki Maintenance 最近摘要、空间元数据），兼容恢复 v1–v20 备份；WorkAction 输出在验收前保持 `held`、接受后才进入待提炼队列，驳回/取消/失败后永久 `excluded`。v20 保留 Agent 执行模式意图，不携带本机完全访问确认；恢复后须在本机重新确认。旧版缺失的高权限审批与执行计划会 fail-closed，legacy Skill 不会被静默绑定到错误来源；升级时会重新派生 WorkAction Raw 状态并清理引用未准入来源的旧 Wiki 页。导出、恢复和删除会阻止仍在运行、等待审批/验收/重试或正在外发的工作，恢复会校验 WorkItem、WorkAction、Task Run、验收证据、Raw 准入、原文件摘要与 checkpoint 的双向关联，避免删除后继续副作用、恢复后重复执行、污染知识、丢失原文件或重复通知。
...
每个附件最多保留 200,000 个字符用于提炼；原文件本体会按字节完整保存在当前 Space 的 `raw/sources/`，不受该字符上限影响，并可从 Raw 详情页下载。提取超限、损坏或格式不支持不会阻止原文件保存，也不影响原消息收录和回复。
