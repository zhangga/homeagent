...
- **Agents**（三栏工作台）：左侧选择 Agent，中间编辑配置，右侧查看真实的 CLI 状态、当前空间/飞书群绑定和该 Agent 最近处理的 Chat / 研究任务运行。Chat 记录会固定归属到实际处理它的 Agent，并可从 Recent runs 进入对应的原始消息详情。支持新建 / 删除，以及配置 **名称、Provider、Instruction（人格，会注入到回答）、Model、推理强度、Visibility、Permission、Workdir**；旧版 Pinned Skill 绑定会保留兼容，但不再限制运行时能力。编辑先保存为草稿，显式发布后才影响未来运行，发布历史不可变并支持通过新版本回滚，避免在途运行被原地改写。
  ...
  - **Skills 自动加载**：飞书群绑定团队空间，空间再绑定 Team Agent；个人空间绑定 Personal Agent。每次普通聊天或任务启动时，HomeAgent 都会扫描当前 Provider 可用的本机 Skill 目录，把完整兼容目录交给 Agent，再由 Agent 按当前请求加载相关 Skill，无需逐个固定。旧 Agent 的 Pinned Skill 仅作为兼容配置保留，不再形成能力白名单。
    Codex 单个 Skill bundle 最多 16 MiB / 50,000 个条目；完整目录的临时副本总预算为 128 MiB / 200,000 个条目，逐个捕获、散列核验后复制。总预算超限会停止整次调用并清理副本，不会删除尾部 Skill 后继续运行。原生调用只为本轮独占、已验证且仅含这些副本的临时根目录授予读取权限，避免逐个路径重复出现在 Windows 命令行中；不会授予本机 Skill 源目录或整个临时目录的访问权。
  ...
  - **群话题不自动降级**：Codex 原生会话预检失败后不会再次发起无话题记忆的普通调用，也不会自动把 `full` 改成 `write`。`full` 配置冲突会明确提示；只读需求选 `read-only`，需要写工作目录才选 `write`，保存并发布后仅影响新运行。两种受限权限仍须通过平台隔离检查，切换权限并不等于修好 Windows 沙箱。重试继续使用原冻结计划；需要采用新权限或全量 Skill 目录时应新建一次请求。
    管理页与回复会区分具体预检原因：退出码 73 表示 Codex 状态目录仍可读，74 表示授权测试目录不可读，75 表示受保护数据目录仍可读。75 不是未登录或缺少 Skill，也不等于需要重新完成 UAC 授权；这台机器未证明根目录拒绝规则生效，必须保持原生话题不可用。仅当明确报告 elevated 后端未就绪时提供对应设置操作；重新检测不得把“CLI 可连接”显示为“原生话题已恢复”。
...
- **数据治理**：按空间导出 `homeagent.space v19` JSON 完整备份（知识页及系统生成的分层地图、原始记录及其动作验收准入状态、按 SHA-256 校验的完整原文件、工作上下文、续作动作/checkpoint/验收审计/策略及其证据关联、人工治理审计、本地 Agent 知识消费反馈及处置记录、撤回标记、任务及 Chat 运行历史、冻结执行计划、Agent 发布历史、审批/重试/通知/用量审计、Skill 证据、相关质量 trace 与封闭的重评记录、提醒、学习计划、主题路线、多来源材料及课程历史、Wiki Maintenance 最近摘要、空间元数据），兼容恢复 v1–v19 备份；WorkAction 输出在验收前保持 `held`、接受后才进入待提炼队列，驳回/取消/失败后永久 `excluded`。旧版缺失的高权限审批与执行计划会 fail-closed，legacy Skill 不会被静默绑定到错误来源；升级时会重新派生 WorkAction Raw 状态并清理引用未准入来源的旧 Wiki 页。导出、恢复和删除会阻止仍在运行、等待审批/验收/重试或正在外发的工作，恢复会校验 WorkItem、WorkAction、Task Run、验收证据、Raw 准入、原文件摘要与 checkpoint 的双向关联，避免删除后继续副作用、恢复后重复执行、污染知识、丢失原文件或重复通知。
