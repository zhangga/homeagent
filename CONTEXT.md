...
Raw.space 始终是发起请求的 Space，Raw.chatId 是实际来源群，可以不同。跨群收录的授权规则是：当前或同话题此前的用户输入
请求提炼群聊历史或明确要求保存原始记录，并声明单一来源群名或 ID；Core 从已保存的用户请求解析默认/显式保存意图和来源，交接 ID 或 chatName 必须匹配该范围。
同话题由既有路由计划的 Space、chatId、rootMessageId 确定；只使用截至当前 Run 的保留输入，当前改群或取消保存覆盖此前要求。
在群消息查询语境中，请求“提炼／提取／整理／总结／汇总”聊天历史默认触发群原文收录；“记录相关信息”“保存相关数据”等显式保存动作也会触发，不要求用户说出“原始／原文／Raw”。单纯查询聊天记录仍不自动收录，明确不保存或只保存总结则退出该流程。
...
管理页的 Dream 运行详情是内存中的观察快照，不是 Raw、执行计划或恢复日志，不进入 Space 归档。
快照按独立运行 ID 展示阶段、来源元数据、知识页与分段进度、时间和结果；仅保存当前运行与最近 50 条结束记录。
不记录 Raw 正文、模型提示词或 Provider 诊断，服务重启后清空；读取详情不会启动模型，也不会重放提炼。
